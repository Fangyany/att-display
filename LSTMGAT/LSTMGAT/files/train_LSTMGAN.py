...
from LSTMGAN import get_fake_traj_rel, get_pred_traj_rel, TrajectoryDiscriminator, Loss, get_model
...
def train(epoch, config, train_loader, generator, discriminator, loss_l2, post_process, optimizer_g, optimizer_d):
    ...
    for i, data in enumerate(train_loader):
        ...
        if g_steps_left > 0:
            ...
            losses_d = discriminator_step(data, out_rel, discriminator, d_loss_fn, optimizer_d)
            d_steps_left -= 1
...
def discriminator_step(data, out_rel, discriminator, d_loss_fn, optimizer_d):
